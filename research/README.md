...
sentinel-wm bundle ../models                 # assemble the portable model bundle
```
...
| `sentinel_wm/bundle.py` | `sentinel-wm bundle` — copies the trained models + scaler + registry into a self-contained `../models/` the backend loads |
...
`registry.json`), `../artifacts/` (working parquet/npz/scaler), and `../models/`
